...
class MergeRules(unittest.TestCase):
    ...
    def test_new_observation_refreshes_eligibility_without_resetting_first_seen(self):
        old = job('u/1', '2026-09-01', last_observed_at='2026-09-01T00:00:00Z',
                  california_eligibility='unverified')
        new = job('u/1', '2026-09-07', last_observed_at='2026-09-07T00:00:00Z',
                  california_eligibility='excluded')
        for left, right in [(old, new), (new, old)]:
            merged = json.loads(ccp.merge_all_jobs(all_jobs_doc(left), all_jobs_doc(right)))['jobs'][0]
            self.assertEqual(merged['first_seen'], '2026-09-01')
            self.assertEqual(merged['california_eligibility'], 'excluded')
