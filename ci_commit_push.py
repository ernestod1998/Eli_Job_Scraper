...
def merge_all_jobs(theirs: str, ours: str) -> str:
    ...
    for job in list(theirs_data.get("jobs", [])) + list(ours_data.get("jobs", [])):
        ...
        if prev is None:
            ...
            # Observation recency determines mutable fields; first_seen only
            # describes when the URL originally entered the feed.
            old_stamp = prev.get('last_observed_at', '')
            new_stamp = job.get('last_observed_at', '')
            winner = job if new_stamp >= old_stamp else prev
            loser = prev if winner is job else job
            merged = {**loser, **winner}
            first = min(_first_seen(prev), _first_seen(job))
            if first != '9999-99-99':
...
def commit_and_push(paths, message, attempts=5, sleep=time.sleep) -> int:
    ...
    for attempt in range(1, attempts + 1):
        ...
        for path in paths:
            ...
            if os.path.basename(path) == 'all_jobs.json' and content and os.path.isfile(os.path.join(SCRIPT_DIR, 'scrape_jobs.py')):
                # Do not resurrect stale or newly excluded remote observations.
                from scrape_jobs import _filter_job_observations
                document = json.loads(content)
                document['jobs'], _, _ = _filter_job_observations(document.get('jobs', []), default_feed='general')
