...
SOURCE_DIAGNOSTICS = []
SMOKE_COLLECTION = os.environ.get("ELI_SMOKE") == "1"
...
def bounded_queries(items):
    ...
    for index, item in enumerate(items):
        if index >= limit or time.monotonic() - COLLECTION_STARTED >= deadline:
            print("  ⚠️  Collection query budget reached; coverage is partial")
            break
        yield item
    if SMOKE_COLLECTION:
        print("  ⚠️  Smoke collection samples source queries; coverage is partial")

def print(*args, **kwargs):
    message = " ".join(str(arg) for arg in args)
    if "⚠️" in message or "⛔" in message:
        SOURCE_DIAGNOSTICS.append(message)
    builtins.print(*args, **kwargs)

def write_source_report(jobs):
    path = os.environ.get("ELI_SOURCE_REPORT_PATH")
    if not path:
        return
    failure = any("⛔" in message or "accumulator failed" in message for message in SOURCE_DIAGNOSTICS)
    status = "failure" if failure else "partial" if SOURCE_DIAGNOSTICS else "success"
    with open(path, "w") as handle:
        json.dump({"status": status, "observations": 0 if failure else len(jobs),
                   "error": "Source reported incomplete collection; see workflow logs." if SOURCE_DIAGNOSTICS else ""}, handle)
...
KEYWORDS = [
    ...
    "management analyst", "business operations analyst", "program analyst",
    "grants analyst", "grants financial", "research financial", "sponsored programs",
    "contract analyst", "contracts analyst", "contracts administrator", "contract administrator",
    "procurement analyst", "purchasing analyst", "buyer", "compliance analyst",
    "contract compliance", "grants compliance", "financial compliance",
]
...
# Posting age filtering is disabled; source acquisition windows stay separate.
MAX_POSTING_AGE_DAYS = None  # Eli keeps listings indefinitely.
...
EXCLUDED_SENIORITY_RE = re.compile(
    r'\b(distinguished|founding|director|'
...
SACRAMENTO_CITIES = ["sacramento", "west sacramento", "davis", "woodland",
    "roseville", "rocklin", "folsom", "rancho cordova", "citrus heights", "elk grove"]

def california_excluded(text: str) -> bool:
    text = text or ""
    if re.search(r'(?:excluding|except|not (?:available|eligible|open|hiring)(?: in| to)?|cannot (?:hire|work)(?: in)?)\s*[:,-]?\s*(?:california|ca)\b|\b(?:california|ca) residents (?:are )?(?:not eligible|excluded)', text, re.I):
        return True
    restriction = re.search(r'(?:must (?:reside|live|be located) in|residents? of)\s+([^.;\n]+)', text, re.I)
    if restriction:
        allowed = restriction.group(1)
        if not re.search(r'\b(california|ca|united states|usa|us)\b', allowed, re.I):
            return True
    return False

def classify_job(job: dict) -> dict:
    """Persist uncertainty rather than infer residency eligibility from remote alone."""
...
def is_remote_us(location: str) -> bool:
    loc = (location or "").lower()
    if "remote" not in loc or california_excluded(loc):
        return False
    if re.search(r'\b(ca|california)\b', loc):
        return True
...
def is_stale_posting(date_value, *, now: datetime | None = None) -> bool:
    """Optional age policy; None keeps every posting regardless of its age."""
...
CURATED_HOLLYWOOD = []  # Legacy interface; Eli uses the independent ATS registry.
...
LINKEDIN_SEARCH_TERMS = ["financial analyst", "budget analyst", "administrative analyst", "compliance analyst", "purchasing analyst", "procurement analyst", "fiscal analyst", "cost analyst", "accounting analyst", "contract analyst", "grants analyst", "FP&A analyst", "program analyst", "buyer"]
WORKDAY_SEARCH_TERMS = LINKEDIN_SEARCH_TERMS[:]
LINKEDIN_LOOKBACK_SECONDS = 172800  # overlapping daily collections
...
def _linkedin_search(terms: list[str], lookback_seconds: int) -> tuple[list[dict], int]:
    ...
    total_raw_cards = 0
    for (loc_name, geo_id), term in bounded_queries(itertools.product(LINKEDIN_LOCATIONS, terms)):
        start = 0
        seen_raw_ids: set[str] = set()
        while start < (25 if SMOKE_COLLECTION else 75):
            time.sleep(REQUEST_DELAY)
            url = (
                "https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search"
                f"?keywords={urllib.parse.quote(term)}"
                f"&location={urllib.parse.quote(loc_name)}"
                f"&geoId={geo_id}" + ("&f_WT=2" if loc_name == "United States" else "") +
...
INDEED_LOOKBACK_HOURS = 48  # Daily collection with overlap
...
def scrape_indeed_recent() -> list:
    ...
    errored_terms = 0
    raw_rows = 0
    for (location, distance), term in bounded_queries(itertools.product(JOBSPY_LOCATIONS, LINKEDIN_SEARCH_TERMS)):
...
ALL_JOBS_PRUNE_DAYS = None  # No automatic age-based master pruning.
...
def _merge_into_all_jobs(observed_jobs: list, rejected_observations: list | None = None) -> int:
    ...
    every retained listing, without an age limit. Returns count added.
    """
    ...
    added = 0
    refresh_fields = {
        "company", "title", "location", "date_posted", "salary", "ats", "url", "california_eligibility", "role_fit", "last_observed_at"
    }
    ...
    with open(path, "w") as f:
        ...
        json.dump({"updated_at": now.strftime("%Y-%m-%d %H:%M UTC"), "jobs": kept},
                  f, separators=(",", ":"))
    print(f"🗂  all_jobs.json: +{added} new, {len(kept)} total (no age expiry)")
    return added
...
def save_jobs_output(jobs: list, *, basename: str, title: str, subtitle: str,
    ...
    if not any("⛔" in message for message in SOURCE_DIAGNOSTICS):
        for job in jobs:
            job["last_observed_at"] = datetime.now(timezone.utc).isoformat()
...
GOV_SEARCH_TERMS = ["financial analyst", "budget analyst", "administrative analyst", "compliance", "purchasing", "procurement", "fiscal analyst", "management analyst", "contracts", "grants"]
...
def scrape_usajobs_recent() -> list:
    ...
    try:
        ...
        for term in bounded_queries(GOV_SEARCH_TERMS):
            time.sleep(REQUEST_DELAY)
            body = json.dumps({
            ...
            if not isinstance(payload, dict) or not isinstance(payload.get("Jobs"), list):
                print("  ⚠️  USAJOBS response has no recognized Jobs array")
                continue
...
def scrape_governmentjobs_recent() -> list:
    ...
    raw_items = 0
    for term in bounded_queries(GOV_SEARCH_TERMS):
        for page in range(1, (1 if SMOKE_COLLECTION else GOVERNMENTJOBS_PAGES) + 1):
            time.sleep(REQUEST_DELAY)
    ...
    if not jobs and raw_items == 0:
        print("  ⛔ NEOGOV returned no recognizable records; preserving previous results")
...
def scrape_calopps_recent() -> list:
    ...
    jobs_by_url: dict[str, dict] = {}
    scanned = 0
    for page in bounded_queries(range(CALOPPS_MAX_PAGES)):
        time.sleep(REQUEST_DELAY)
    ...
    if not jobs and scanned == 0:
        print("  ⛔ CalOpps returned no recognizable records; preserving previous results")
...
def scrape_calcareers_recent() -> list:
    ...
    if not jobs and (parsed_total == 0 or not reached):
        print("  ⛔ CalCareers returned no recognizable records; preserving previous results")
...
def scrape_registry_recent() -> dict:
    ...
    if not registry.get("boards"):
        print("  ⛔ ATS registry has no configured boards")
    ...
    if result.get("boards_failed"):
        print("  ⚠️  Registry boards failed; collection is partial")
    print(
        "🗃  ATS registry: "
...
def save_registry_results(result: dict) -> None:
    save_jobs_output(
        ...
        title="🗃 Direct ATS Registry — Finance / Administration / Procurement Roles",
...
REFILTER_RENDER_CONFIG = {
    ...
    "registry_jobs": ("🗃 Direct ATS Registry — Finance / Administration / Procurement Roles", "Verified public ATS boards", "#f59e0b", "No eligible registry roles in this shard."),
}
